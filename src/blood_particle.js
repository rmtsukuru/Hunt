...
const BLOOD_PARTICLE_SPEED = 5;
const BLOOD_PARTICLE_SPEED_VARIANCE = 7;
const BLOOD_PARTICLE_SPRAY_FRAMES = 0.1 * FPS;
...
function BloodParticle(x, y) {
    ...
    this.xVelocity = BLOOD_PARTICLE_SPEED / 2 + BLOOD_PARTICLE_SPEED_VARIANCE * (Math.random() - 0.5);
    if (Math.random() < 0.5) {
        this.xVelocity *= -1;
    }
